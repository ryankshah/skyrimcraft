...
public class DwarvenSpiderEntity extends PathfinderMob implements GeoEntity
{
    private static final EntityDataAccessor<Integer> PREV_ANIMATION_STATE = SynchedEntityData.defineId(DwarvenSpiderEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> ANIMATION_STATE = SynchedEntityData.defineId(DwarvenSpiderEntity.class, EntityDataSerializers.INT);
...
//    private NearestAttackableTargetGoal<? extends LivingEntity> sprintToNearestAnimalGoal;
    ...
    public DwarvenSpiderEntity(EntityType<? extends PathfinderMob> type, Level worldIn) {
        super(type, worldIn);
        this.noCulling = true;
        this.xpReward = 5;
//        this.setMaxUpStep(1.25f); // 1.5 works.. but does 1.25f? if so then this comment may still be here xox
    ...
    protected void registerGoals() {
        meleeGoal = new MeleeAttackGoal(this, 1.2D, false) {
            ...
            public void stop() {
                super.stop();
                DwarvenSpiderEntity.this.setAnimationState(0);
            }
            ...
            protected void checkAndPerformAttack(LivingEntity pTarget) {
                if (this.canPerformAttack(pTarget)) {
                    if (getTicksUntilNextAttack() <= 0) {
                        ...
                        if (DwarvenSpiderEntity.this.getAnimationState() != 3 || DwarvenSpiderEntity.this.getAnimationState() != 4)
                            DwarvenSpiderEntity.this.setAnimationState(random.nextBoolean() ? 3 : 4);
        ...
        walkingGoal = new WaterAvoidingRandomStrollGoal(this, 1.0D) {
            ...
            public void stop() {
                super.stop();
                DwarvenSpiderEntity.this.setAnimationState(DwarvenSpiderEntity.this.getPrevAnimationState());
            }

            @Override
            public void tick() {
                super.tick();
                if(DwarvenSpiderEntity.this.getAnimationState() != 2)
                    DwarvenSpiderEntity.this.setAnimationState(2);
            }
        };
...
//        this.targetSelector.addGoal(2, new SprintToNearestAttackableTargetGoal<>(this, Animal.class, true));
    ...
    private <E extends DwarvenSpiderEntity> PlayState spiderController(final software.bernie.geckolib.core.animation.AnimationState<DwarvenSpiderEntity> event) {
        AnimationController<DwarvenSpiderEntity> controller = event.getController();
